...
> **Status (2026-07-12)**: All 76 tasks implemented. The full module, data-driven change
> catalog, Pester v5 suite (448 tests passing), PSScriptAnalyzer (0 findings on our code),
> both GitHub Actions workflows, and all docs are complete. Unit tests mock DISM/oscdimg/reg,
> so the code is fully verified on Linux/CI-lint; the **end-to-end ISO build is Windows-only**
> and is exercised by `.github/workflows/build-image.yml` on `windows-latest` /
> `windows-11-arm` runners (or a local Windows admin machine).
...
- [X] T001 Create the repository directory structure per plan (`src/WindowsIsoMaker/Public/`,
  `src/WindowsIsoMaker/Private/`, `config/`, `templates/autounattend/`, `tests/`, `docs/`,
  `vendor/fido/`, `.github/workflows/`)
- [X] T002 [P] Create the pinned module manifest `src/WindowsIsoMaker/WindowsIsoMaker.psd1`
...
- [X] T003 [P] Create the module loader `src/WindowsIsoMaker/WindowsIsoMaker.psm1` that sets
  `Set-StrictMode -Version Latest`, dot-sources every `Private/*.ps1` then `Public/*.ps1`,
  and exports only the public functions
- [X] T004 [P] Create `PSScriptAnalyzerSettings.psd1` at repo root (enable default rule set,
  disallow aliases/positional-heavy calls, require approved verbs; used by both local runs
  and `ci.yml`)
- [X] T005 [P] Vendor pinned Fido under `vendor/fido/`: add `Fido.ps1` (record the pinned
  upstream commit), `LICENSE` (GPLv3 preserved), and `VERSION` (pinned commit/tag +
  upstream URL) per plan's GPLv3 isolation boundary
- [X] T006 [P] Create the root entry script `build.ps1` (thin dispatcher: `Set-StrictMode`,
  ...
  Principle I/V)
- [X] T007 [P] Create the Pester v5 test bootstrap `tests/PesterConfiguration.ps1` (v5
...
- [X] T008 [P] Implement structured logging helper
...
- [X] T013 [P] Implement run-report writer
...
- [X] T014 Implement preconditions gate
  ...
  T009, T010
- [X] T015 [P] Implement catalog-selection resolver
...
- [X] T016 [P] Write resolver tests FIRST in `tests/Resolve-CatalogSelection.Tests.ps1`
...
- [X] T017 Write change-catalog **schema v2** tests FIRST in `tests/Catalog.Schema.Tests.ps1`:
...
- [X] T018 [P] Create provisioned-appx removal catalog `config/catalog.appx.psd1` (default
...
- [X] T019 [P] Create Windows-capability/optional-feature catalog
  ...
  **WSL** entries `feature-wsl` (`Action=EnableOptionalFeature`,
  ...
  FR-021/FR-025
- [X] T020 [P] Create registry-tweak catalog `config/catalog.registry.psd1` (every entry
...
- [X] T021 [P] Create the default build config **v2** `config/build.config.psd1`
...
- [X] T022 Write config loader **v2** tests FIRST in `tests/Get-BuildConfiguration.Tests.ps1`
...
- [X] T023 Implement `src/WindowsIsoMaker/Public/Get-BuildConfiguration.ps1` (config file is
...
- [X] T024 [P] [US1] Write `tests/Get-Windows11Iso.Tests.ps1` (mocked Fido: arg mapping,
  `IsoPath` override path, unavailable edition/lang/release combo → terminating error)
- [X] T025 [P] [US1] Write `tests/Remove-Bloatware.Tests.ps1` (mocked
...
- [X] T026 [P] [US1] Write `tests/Set-RegistryTweaks.Tests.ps1` (mocked hive load/unload +
  `Set-ItemProperty`: Recall+Widgets applied, `-WhatIf` reports only, hives always unloaded
  on failure)
- [X] T027 [P] [US1] Write `tests/Enable-WindowsFeature.Tests.ps1` (mocked
  ...
  opt-in) — FR-025
- [X] T028 [P] [US1] Write `tests/Invoke-CatalogEntry.Tests.ps1` (Action routing:
...
- [X] T029 [P] [US1] Write `tests/New-AutounattendXml.Tests.ps1` (`processorArchitecture`
  ...
  FR-027
- [X] T030 [P] [US1] Write `tests/Export-ImageBom.Tests.ps1` (BOM enumerates every `Applied`
  ...
  FR-029/SC-012
- [X] T031 [P] [US1] Write `tests/Invoke-IsoBuild.Tests.ps1` (mocked pipeline: preconditions
...
- [X] T032 [P] [US1] Write `tests/Test-ImageIntegrity.Tests.ps1` (missing boot file → fail;
  structural checks; `BootTest` off by default)

### Implementation for User Story 1

- [X] T033 [P] [US1] Implement `src/WindowsIsoMaker/Public/Get-Windows11Iso.ps1` (Fido
  wrapper: array-built args, `IsoPath` override, hash/integrity recording, returns
  `BaseImage`) — FR-001/FR-002/FR-020
- [X] T034 [P] [US1] Implement `src/WindowsIsoMaker/Public/Expand-WindowsImage.ps1` (extract
  ISO media to working dir, locate `sources/install.wim|.esd`)
- [X] T035 [P] [US1] Implement `src/WindowsIsoMaker/Public/Mount-WindowsBuildImage.ps1`
  (resolve edition→index, mount via DISM, return `MountedImage`, guard cleanup)
- [X] T036 [US1] Implement `src/WindowsIsoMaker/Public/Remove-Bloatware.ps1` (handler for
...
- [X] T037 [US1] Implement `src/WindowsIsoMaker/Public/Set-RegistryTweaks.ps1` (handler for
  ...
  T020
- [X] T038 [US1] Implement `src/WindowsIsoMaker/Public/Enable-WindowsFeature.ps1` (handler for
  ...
  T010, T019 — FR-025
- [X] T039 [US1] Implement `src/WindowsIsoMaker/Public/Invoke-CatalogEntry.ps1` (Action
...
- [X] T040 [P] [US1] Create the per-arch Autounattend template(s) under
...
- [X] T041 [P] [US1] Implement `src/WindowsIsoMaker/Public/New-AutounattendXml.ps1` (render
...
- [X] T042 [P] [US1] Implement `src/WindowsIsoMaker/Public/New-BootableIso.ps1` (oscdimg
...
- [X] T043 [P] [US1] Implement `src/WindowsIsoMaker/Public/Compress-BuildArtifact.ps1`
  (compress ISO, name `Windows11-<edition>-<arch>-<release>.<ext>`, compute sha256, return
  `OutputImageArtifact`)
- [X] T044 [P] [US1] Implement `src/WindowsIsoMaker/Public/Test-ImageIntegrity.ps1`
...
- [X] T045 [P] [US1] Implement `src/WindowsIsoMaker/Public/Export-ImageBom.ps1` (derive the
...
- [X] T046 [US1] Implement `src/WindowsIsoMaker/Public/Invoke-IsoBuild.ps1` orchestrator
...
- [X] T047 [US1] Verify `FunctionsToExport` in `src/WindowsIsoMaker/WindowsIsoMaker.psd1`
...
- [X] T048 [P] [US2] Add arm64 boot-data selection to
  ...
  T042
- [X] T049 [P] [US2] Add `tests/New-BootableIso.Tests.ps1` (mocked oscdimg: arch→boot-arg
...
- [X] T050 [US2] Create `docs/ci.md` (runner disk/time limits, ADK Deployment Tools install,
  arm64 tooling verification, documented skip-heavy-build path, provenance/SBOM/Azure-upload
  overview + artifact-size rationale)
- [X] T051 [US2] Create `.github/workflows/build-image.yml` (`workflow_dispatch` ONLY; inputs
...
- [X] T052 [US2] Add build provenance to `.github/workflows/build-image.yml`: publish the
  ...
  FR-028/SC-009
- [X] T053 [US2] Add an optional Azure Blob upload job/step to
...
- [X] T054 [US3] Create `.github/workflows/ci.yml` (triggers `push` + `pull_request` on
...
- [X] T055 [US3] Add a repository/tooling SBOM step to `.github/workflows/ci.yml` using
  `anchore/sbom-action` (SHA-pinned) producing a CycloneDX SBOM and uploading it as an
  artifact. Depends on T054 — FR-029
- [X] T056 [US3] Add negative-path assertions to `tests/Catalog.Schema.Tests.ps1` proving a
...
- [X] T057 [US3] Verify `renovate.json5` at repo root governs dependency updates: extends the
...
- [X] T058 [P] [US4] Add the opt-in removal catalog entries `remove-edge` and `remove-onedrive`
  ...
  — FR-008/FR-024
- [X] T059 [US4] Verify end-to-end opt-in enablement of `remove-edge`/`remove-onedrive`/
...
- [X] T060 [US4] Wire resolved edition/language/release/arch pass-through from
...
- [X] T061 [P] [US4] Extend `tests/Get-BuildConfiguration.Tests.ps1` with customization cases
...
- [X] T062 [US5] Implement `-WhatIf`/preview path in
...
- [X] T063 [US5] Implement idempotency detection (read current state → mark `AlreadyApplied`)
  ...
  FR-017/SC-007
- [X] T064 [US5] Add failure-path cleanup to
  ...
  — FR-005
- [X] T065 [P] [US5] Add `Reversal` notes to every reversible entry across
  `config/catalog.registry.psd1`, `config/catalog.appx.psd1`,
  `config/catalog.capabilities.psd1` — FR-018
- [X] T066 [P] [US5] Add `tests/Invoke-IsoBuild.Preview.Tests.ps1` (preview modifies nothing;
...
- [X] T067 [P] Create `docs/usage.md` (quick-start, config-file-first interface, single-command
  local build, `Profile`/`EnableCatalogId` examples) — SC-008
- [X] T068 [P] Create `docs/change-rationale.md` (human-readable rendering of every catalog
  entry's What/Why/Citation/**EvidenceGrade**/Reversal) — Principle II
- [X] T069 [P] Create `docs/evidence-grading.md` (the 1/2/3 grading rubric — MS official /
  reputable vendor / community — and the grade-3-may-not-be-DefaultEnabled gate) —
  FR-026/SC-010
- [X] T070 [P] Create `docs/wsl.md` (WSL as an opt-in catalog entry; offline enables
  `Microsoft-Windows-Subsystem-Linux` + `VirtualMachinePlatform`; kernel + distro install
  online on first boot) — FR-025
- [X] T071 [P] Create `docs/autounattend.md` (per-arch `processorArchitecture`; OOBE skip;
  MS-account bypass + local account default-on/toggleable; locale/keyboard/timezone; disk;
  FirstLogon/SetupComplete) — FR-027
- [X] T072 [P] Create `docs/provenance-bom.md` (verify `SHA256SUMS`; `gh attestation verify`
  the SLSA provenance; read the Image BOM; locate the repo CycloneDX SBOM) — FR-028/FR-029
- [X] T073 [P] Create `docs/azure-upload.md` (optional OIDC Azure Blob upload: required repo
  vars, no stored secrets, fallback to workflow artifact, ~5–7 GB artifact-size rationale) —
  FR-030
- [X] T074 Run a clean `Invoke-ScriptAnalyzer -Settings ./PSScriptAnalyzerSettings.psd1
  -Recurse` pass and resolve all findings across `src/`, `tests/`, `build.ps1`
- [X] T075 Execute quickstart validation Scenarios A–G from
...
- [X] T076 [P] Update root `README.md` with a project overview and pointers to `docs/usage.md`,
